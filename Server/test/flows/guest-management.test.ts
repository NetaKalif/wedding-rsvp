...
const addGuest = (name: string, phone: string) =>
  axios.patch(`${REAL_SERVER}/addGuests`, {
    userID: USER_ID,
...
const getAllGuests = async (): Promise<Array<{ id: number; name: string }>> => {
  const { data } = await axios.post(`${REAL_SERVER}/guestsList`, { userID: USER_ID });
  return data;
};
